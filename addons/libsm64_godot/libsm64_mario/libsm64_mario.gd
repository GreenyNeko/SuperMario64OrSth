...
@export var disableInputs = false
...
func _process(delta: float) -> void:
	...
	_mesh_instance.visible = modelVisible
...
func _tick() -> void:
	...
	var mario_inputs = LibSM64MarioInputs.new()
	if not disableInputs:
		mario_inputs = _make_mario_inputs()

	_mario_interpolator.mario_state_previous = _mario_interpolator.mario_state_current
	_mario_interpolator.array_mesh_triangles_previous = _mario_interpolator.array_mesh_triangles_current

	if stationary and offsetNode:
		LibSM64.set_mario_position(id, offsetNode.position)#+ Vector3(11.704, -2.6, -49.553))
	...
	_mario_interpolator.array_mesh_triangles_current = mario_tick_output[1]
		...
		#_mario_interpolator.mario_state_current.position = offsetNode.position
